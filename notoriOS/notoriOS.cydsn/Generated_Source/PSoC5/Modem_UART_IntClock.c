/*******************************************************************************
* File Name: Modem_UART_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Modem_UART_IntClock_Start
********************************************************************************
*
* Summary:
...
void Modem_UART_IntClock_Start(void) 
{
    ...
    Modem_UART_IntClock_CLKEN |= Modem_UART_IntClock_CLKEN_MASK;
	Modem_UART_IntClock_CLKSTBY |= Modem_UART_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Modem_UART_IntClock_Stop
********************************************************************************
*
* Summary:
...
void Modem_UART_IntClock_Stop(void) 
{
    ...
    Modem_UART_IntClock_CLKEN &= (uint8)(~Modem_UART_IntClock_CLKEN_MASK);
	Modem_UART_IntClock_CLKSTBY &= (uint8)(~Modem_UART_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Modem_UART_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Modem_UART_IntClock_CLKEN & Modem_UART_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Modem_UART_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Modem_UART_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Modem_UART_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Modem_UART_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Modem_UART_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Modem_UART_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Modem_UART_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Modem_UART_IntClock_GetSourceRegister();
    uint16 oldDivider = Modem_UART_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Modem_UART_IntClock_CLKEN & Modem_UART_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Modem_UART_IntClock_DIV_PTR, clkDivider);
                Modem_UART_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Modem_UART_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Modem_UART_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Modem_UART_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Modem_UART_IntClock_DIV_PTR, clkDivider);
				Modem_UART_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Modem_UART_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Modem_UART_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Modem_UART_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Modem_UART_IntClock_MOD_SRC |= modeBitMask & (uint8)Modem_UART_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Modem_UART_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Modem_UART_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Modem_UART_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Modem_UART_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Modem_UART_IntClock_MOD_SRC & (uint8)(Modem_UART_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Modem_UART_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Modem_UART_IntClock_GetDividerRegister();
    uint8 oldSrc = Modem_UART_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Modem_UART_IntClock_MOD_SRC |= CYCLK_SSS;
        Modem_UART_IntClock_MOD_SRC =
            (Modem_UART_IntClock_MOD_SRC & (uint8)(~Modem_UART_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Modem_UART_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Modem_UART_IntClock_MOD_SRC & Modem_UART_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Modem_UART_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Modem_UART_IntClock_PHASE = clkPhase & Modem_UART_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Modem_UART_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Modem_UART_IntClock_PHASE & Modem_UART_IntClock_PHASE_MASK;
}
