...
CY_ISR(emFile_SPI0_TX_ISR)
{
    ...
    /* `#END` */
        ...
        while(emFile_SPI0_txBufferRead != emFile_SPI0_txBufferWrite)
        {
            ...
            emFile_SPI0_swStatusTx = tmpStatus;

            if(0u != (emFile_SPI0_swStatusTx & emFile_SPI0_STS_TX_FIFO_NOT_FULL))
            {
                if(0u == emFile_SPI0_txBufferFull)
                {
...
CY_ISR(emFile_SPI0_RX_ISR)
{
    ...
    /* `#END` */
        ...
        emFile_SPI0_swStatusRx = tmpStatus;

        /* Check if RX data FIFO has some data to be moved into the RX Buffer */
        while(0u != (emFile_SPI0_swStatusRx & emFile_SPI0_STS_RX_FIFO_NOT_EMPTY))
        {
            rxData = CY_GET_REG8(emFile_SPI0_RXDATA_PTR);

            /* Set next pointer. */
            emFile_SPI0_rxBufferWrite++;
            if(emFile_SPI0_rxBufferWrite >= emFile_SPI0_RX_BUFFER_SIZE)
            {
