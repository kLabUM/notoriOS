/*******************************************************************************
* File Name: Downstream_Clock_IMO.c
* Version 2.20
*
*  Description:
...
#include "Downstream_Clock_IMO.h"
...
/*******************************************************************************
* Function Name: Downstream_Clock_IMO_Start
********************************************************************************
*
* Summary:
...
void Downstream_Clock_IMO_Start(void) 
{
    ...
    Downstream_Clock_IMO_CLKEN |= Downstream_Clock_IMO_CLKEN_MASK;
	Downstream_Clock_IMO_CLKSTBY |= Downstream_Clock_IMO_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Downstream_Clock_IMO_Stop
********************************************************************************
*
* Summary:
...
void Downstream_Clock_IMO_Stop(void) 
{
    ...
    Downstream_Clock_IMO_CLKEN &= (uint8)(~Downstream_Clock_IMO_CLKEN_MASK);
	Downstream_Clock_IMO_CLKSTBY &= (uint8)(~Downstream_Clock_IMO_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Downstream_Clock_IMO_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Downstream_Clock_IMO_CLKEN & Downstream_Clock_IMO_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Downstream_Clock_IMO_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Downstream_Clock_IMO_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Downstream_Clock_IMO__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Downstream_Clock_IMO_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Downstream_Clock_IMO_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Downstream_Clock_IMO_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Downstream_Clock_IMO_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Downstream_Clock_IMO_GetSourceRegister();
    uint16 oldDivider = Downstream_Clock_IMO_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Downstream_Clock_IMO_CLKEN & Downstream_Clock_IMO_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Downstream_Clock_IMO_DIV_PTR, clkDivider);
                Downstream_Clock_IMO_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Downstream_Clock_IMO_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Downstream_Clock_IMO_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Downstream_Clock_IMO_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Downstream_Clock_IMO_DIV_PTR, clkDivider);
				Downstream_Clock_IMO_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Downstream_Clock_IMO_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Downstream_Clock_IMO_DIV_PTR);
}


/*******************************************************************************
* Function Name: Downstream_Clock_IMO_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Downstream_Clock_IMO_MOD_SRC |= modeBitMask & (uint8)Downstream_Clock_IMO_MODE_MASK;
}


/*******************************************************************************
* Function Name: Downstream_Clock_IMO_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Downstream_Clock_IMO_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Downstream_Clock_IMO_MODE_MASK));
}


/*******************************************************************************
* Function Name: Downstream_Clock_IMO_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Downstream_Clock_IMO_MOD_SRC & (uint8)(Downstream_Clock_IMO_MODE_MASK);
}


/*******************************************************************************
* Function Name: Downstream_Clock_IMO_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Downstream_Clock_IMO_GetDividerRegister();
    uint8 oldSrc = Downstream_Clock_IMO_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Downstream_Clock_IMO_MOD_SRC |= CYCLK_SSS;
        Downstream_Clock_IMO_MOD_SRC =
            (Downstream_Clock_IMO_MOD_SRC & (uint8)(~Downstream_Clock_IMO_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Downstream_Clock_IMO_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Downstream_Clock_IMO_MOD_SRC & Downstream_Clock_IMO_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Downstream_Clock_IMO_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Downstream_Clock_IMO_PHASE = clkPhase & Downstream_Clock_IMO_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Downstream_Clock_IMO_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Downstream_Clock_IMO_PHASE & Downstream_Clock_IMO_PHASE_MASK;
}
