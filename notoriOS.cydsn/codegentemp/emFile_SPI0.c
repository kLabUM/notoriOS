...
#include "emFile_SPI0_PVT.h"
    ...
    volatile uint8 emFile_SPI0_txBuffer[emFile_SPI0_TX_BUFFER_SIZE] = {0u};
    volatile uint8 emFile_SPI0_txBufferFull;
    volatile uint8 emFile_SPI0_txBufferRead;
    volatile uint8 emFile_SPI0_txBufferWrite;
#endif /* (emFile_SPI0_TX_SOFTWARE_BUF_ENABLED) */

#if(emFile_SPI0_RX_SOFTWARE_BUF_ENABLED)
    volatile uint8 emFile_SPI0_rxBuffer[emFile_SPI0_RX_BUFFER_SIZE] = {0u};
    volatile uint8 emFile_SPI0_rxBufferFull;
    volatile uint8 emFile_SPI0_rxBufferRead;
    volatile uint8 emFile_SPI0_rxBufferWrite;
#endif /* (emFile_SPI0_RX_SOFTWARE_BUF_ENABLED) */

uint8 emFile_SPI0_initVar = 0u;

volatile uint8 emFile_SPI0_swStatusTx;
volatile uint8 emFile_SPI0_swStatusRx;
...
void emFile_SPI0_Init(void) 
{
    ...
    /* Init TX ISR  */
    #if(0u != emFile_SPI0_INTERNAL_TX_INT_ENABLED)
        CyIntDisable         (emFile_SPI0_TX_ISR_NUMBER);
        CyIntSetPriority     (emFile_SPI0_TX_ISR_NUMBER,  emFile_SPI0_TX_ISR_PRIORITY);
        (void) CyIntSetVector(emFile_SPI0_TX_ISR_NUMBER, &emFile_SPI0_TX_ISR);
    #endif /* (0u != emFile_SPI0_INTERNAL_TX_INT_ENABLED) */

    /* Init RX ISR  */
    #if(0u != emFile_SPI0_INTERNAL_RX_INT_ENABLED)
        CyIntDisable         (emFile_SPI0_RX_ISR_NUMBER);
        CyIntSetPriority     (emFile_SPI0_RX_ISR_NUMBER,  emFile_SPI0_RX_ISR_PRIORITY);
        (void) CyIntSetVector(emFile_SPI0_RX_ISR_NUMBER, &emFile_SPI0_RX_ISR);
    #endif /* (0u != emFile_SPI0_INTERNAL_RX_INT_ENABLED) */
    ...
    (void) emFile_SPI0_ReadTxStatus(); /* Clear Tx status and swStatusTx */
    (void) emFile_SPI0_ReadRxStatus(); /* Clear Rx status and swStatusRx */
...
void emFile_SPI0_Stop(void) 
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    emFile_SPI0_TX_STATUS_ACTL_REG &= ((uint8) ~emFile_SPI0_INT_ENABLE);
    emFile_SPI0_RX_STATUS_ACTL_REG &= ((uint8) ~emFile_SPI0_INT_ENABLE);
    CyExitCriticalSection(enableInterrupts);
...
*  emFile_SPI0_txBuffer[emFile_SPI0_TX_BUFFER_SIZE] - used to store
...
void emFile_SPI0_WriteTxData(uint8 txData) 
{
        ...
        do
        {
            tmpTxBufferRead = emFile_SPI0_txBufferRead;
            if(0u == tmpTxBufferRead)
            {
                tmpTxBufferRead = (emFile_SPI0_TX_BUFFER_SIZE - 1u);
            }
            else
            {
                tmpTxBufferRead--;
        ...
        tempStatus = emFile_SPI0_GET_STATUS_TX(emFile_SPI0_swStatusTx);
        emFile_SPI0_swStatusTx = tempStatus;


        if((emFile_SPI0_txBufferRead == emFile_SPI0_txBufferWrite) &&
           (0u != (emFile_SPI0_swStatusTx & emFile_SPI0_STS_TX_FIFO_NOT_FULL)))
        {
        ...
        else
        {
            ...
            if(emFile_SPI0_txBufferWrite >= emFile_SPI0_TX_BUFFER_SIZE)
            {
                emFile_SPI0_txBufferWrite = 0u;
            }
            ...
            if(emFile_SPI0_txBufferWrite == emFile_SPI0_txBufferRead)
            {
                ...
                if(emFile_SPI0_txBufferRead >= emFile_SPI0_TX_BUFFER_SIZE)
                {
                    emFile_SPI0_txBufferRead = 0u;
                }
        ...
        while(0u == (emFile_SPI0_TX_STATUS_REG & emFile_SPI0_STS_TX_FIFO_NOT_FULL))
        {
...
*  emFile_SPI0_rxBuffer[emFile_SPI0_RX_BUFFER_SIZE] - used to store
*  received data.
*
* Theory:
...
uint8 emFile_SPI0_ReadRxData(void) 
{
    uint8 rxData;
        ...
        {
            if(0u == emFile_SPI0_rxBufferFull)
            {
                emFile_SPI0_rxBufferRead++;
                if(emFile_SPI0_rxBufferRead >= emFile_SPI0_RX_BUFFER_SIZE)
                {
                    emFile_SPI0_rxBufferRead = 0u;
                }
            }
...
uint8 emFile_SPI0_GetRxBufferSize(void) 
{
    uint8 size;
        ...
        {
            size = (emFile_SPI0_RX_BUFFER_SIZE - emFile_SPI0_rxBufferRead) + emFile_SPI0_rxBufferWrite;
        }
        ...
        size = (0u != (emFile_SPI0_RX_STATUS_REG & emFile_SPI0_STS_RX_FIFO_NOT_EMPTY)) ? 1u : 0u;
...
uint8  emFile_SPI0_GetTxBufferSize(void) 
{
    uint8 size;
        ...
        {
            size = (emFile_SPI0_TX_BUFFER_SIZE - emFile_SPI0_txBufferRead) + emFile_SPI0_txBufferWrite;
        }
        ...
        if(0u != (size & emFile_SPI0_STS_TX_FIFO_EMPTY))
        {
            size = 0u;
        }
        else if(0u != (size & emFile_SPI0_STS_TX_FIFO_NOT_FULL))
        {
            size = 1u;
        }
        else
        {
            size = emFile_SPI0_FIFO_SIZE;
        }
...
void emFile_SPI0_ClearRxBuffer(void) 
{
    ...
    while(0u !=(emFile_SPI0_RX_STATUS_REG & emFile_SPI0_STS_RX_FIFO_NOT_EMPTY))
    {
        (void) CY_GET_REG8(emFile_SPI0_RXDATA_PTR);
    }
...
void emFile_SPI0_ClearTxBuffer(void) 
{
    ...
    enableInterrupts = CyEnterCriticalSection();
    /* Clear TX FIFO */
    emFile_SPI0_AUX_CONTROL_DP0_REG |= ((uint8)  emFile_SPI0_TX_FIFO_CLR);
    emFile_SPI0_AUX_CONTROL_DP0_REG &= ((uint8) ~emFile_SPI0_TX_FIFO_CLR);

    #if(emFile_SPI0_USE_SECOND_DATAPATH)
        /* Clear TX FIFO for 2nd Datapath */
        emFile_SPI0_AUX_CONTROL_DP1_REG |= ((uint8)  emFile_SPI0_TX_FIFO_CLR);
        emFile_SPI0_AUX_CONTROL_DP1_REG &= ((uint8) ~emFile_SPI0_TX_FIFO_CLR);
    ...
    CyExitCriticalSection(enableInterrupts);
        ...
        emFile_SPI0_TX_STATUS_MASK_REG &= ((uint8) ~emFile_SPI0_STS_TX_FIFO_NOT_FULL);
    ...
    *******************************************************************************/
    ...
    {
        emFile_SPI0_CONTROL_REG &= ((uint8) ~emFile_SPI0_CTRL_TX_SIGNAL_EN);
    }
                                                                          ...
                                                                          
{
    ...
    while(byteCount > 0u)
    {
        emFile_SPI0_WriteTxData(buffer[bufIndex]);
        bufIndex++;
        byteCount--;
    }
}
...
{
    emFile_SPI0_TX_STATUS_MASK_REG  = (intSrc & ((uint8) ~emFile_SPI0_STS_SPI_IDLE));
    emFile_SPI0_RX_STATUS_MASK_REG  =  intSrc;
}
...
uint8 emFile_SPI0_ReadStatus(void) 
{
    ...
    #if(emFile_SPI0_TX_SOFTWARE_BUF_ENABLED || emFile_SPI0_RX_SOFTWARE_BUF_ENABLED)

        emFile_SPI0_DisableInt();

        tmpStatus  = emFile_SPI0_GET_STATUS_RX(emFile_SPI0_swStatusRx);
        tmpStatus |= emFile_SPI0_GET_STATUS_TX(emFile_SPI0_swStatusTx);
        tmpStatus &= ((uint8) ~emFile_SPI0_STS_SPI_IDLE);
    ...
    #endif /* (emFile_SPI0_TX_SOFTWARE_BUF_ENABLED || emFile_SPI0_RX_SOFTWARE_BUF_ENABLED) */
